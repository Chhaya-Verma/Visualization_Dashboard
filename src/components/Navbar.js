...
const Navbar = ({ toggleTheme, isDarkMode, userName}) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      const preferencesResponse = await fetch(`https://visualization-dashboard-7gxv.onrender.com/preferences?userId=${userId}`);
      const preferences = await preferencesResponse.json();
  
      if (preferences.length > 0) {
        await fetch(`https://visualization-dashboard-7gxv.onrender.com/preferences/${preferences[0].id}`, {
          method: "DELETE",
        });
      ...
      const dbResponse = await fetch(`https://visualization-dashboard-7gxv.onrender.com/users/${userId}`, { method: "DELETE" });
