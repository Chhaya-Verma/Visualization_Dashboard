...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (userId && !isSharedView) {
      ...
      fetch(`https://visualization-dashboard-7gxv.onrender.com/users/${userId}`)
  ...
  useEffect(() => {
    fetch("https://visualization-dashboard-7gxv.onrender.com/data")
      .then((response) => response.json())
  ...
  useEffect(() => {
    if (isSharedView && sharedUserId) {
      fetch(`https://visualization-dashboard-7gxv.onrender.com/preferences?userId=${sharedUserId}`)
        .then((response) => response.json())
        .then((preferences) => {
  ...
  useEffect(() => {
    ...
    const fetchAndSavePreferences = async () => {
      try {
        const response = await fetch(`https://visualization-dashboard-7gxv.onrender.com/preferences?userId=${userId}`);
        ...
        if (isMounted) {
          if (preferences.length > 0) {
            ...
            if (!sharedFeature) {
              const createResponse = await fetch("https://visualization-dashboard-7gxv.onrender.com/preferences", {
                method: "POST",
        ...
        if (preferences.length > 0) {
          await Promise.all(
            preferences
              ...
              .map((pref) =>
                fetch(`https://visualization-dashboard-7gxv.onrender.com/preferences/${pref.id}`, {
                  method: "PUT",
                  headers: { "Content-Type": "application/json" },
  ...
  const clearPreferences = () => {
    ...
    fetch(`https://visualization-dashboard-7gxv.onrender.com/preferences?userId=${userId}`)
      .then((response) => response.json())
      .then((preferences) => {
        if (preferences.length > 0) {
